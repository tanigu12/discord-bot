...
const createMockResult = (
  overrides: Partial<TranslationProcessingResult> = {}
...
describe('TranslationFormatter', () => {
  ...
  describe('addLineBreaks', () => {
    ...
    it('should break text at word boundaries when over 100 characters', () => {
      const addLineBreaks = getAddLineBreaks(formatter);
      const longText =
        'This is a longer sentence that contains multiple words and should be broken at an appropriate word boundary when it exceeds the one hundred character limit.';
    ...
    it('should handle mixed content with long words and normal text', () => {
      const addLineBreaks = getAddLineBreaks(formatter);
      const mixedText =
        'This is normal text with a supercalifragilisticexpialidocious' +
        'A'.repeat(50) +
        ' word that should be broken properly.';
    ...
    it('should respect custom line length limit', () => {
      const addLineBreaks = getAddLineBreaks(formatter);
      const text =
        'This is a test sentence that should be broken at fifty characters exactly when we specify that limit.';
    ...
    it('should preserve multiple spaces between words', () => {
      ...
      const textWithSpaces =
        'This  has    multiple   spaces   between words that should be preserved in the output text.';
  ...
  describe('generateCompleteMessage integration', () => {
    it('should apply line breaks to translation content', () => {
      ...
      const longTranslation =
        'This is a very long translation that contains many words and should be broken into multiple lines because it significantly exceeds the one hundred character limit.';
      ...
      const lines = result.split('\n');
      const translationLines = lines.filter((line: string) =>
        line.includes('significantly exceeds')
      );
    ...
    it('should apply line breaks to evaluation content', () => {
      const mockUser = createMockUser();
      const longEvaluation =
        'This is a very detailed evaluation of your translation attempt that provides comprehensive feedback and suggestions for improvement, covering multiple aspects of grammar, vocabulary, and natural expression in English.';
      ...
      const mockResult = createMockResult({
        ...
        translationEvaluation: {
          ...
          studyPoints: [
            'Very long study point that needs to be broken because it contains detailed explanations',
          ],
          improvements:
            'Long improvement suggestion that should also be broken into readable chunks for better user experience',
        },
      });
    ...
    it('should apply line breaks to questions and answers', () => {
      const mockUser = createMockUser();
      const longQuestion =
        'What is the most important aspect of learning English grammar that you think students should focus on when they are trying to improve their writing skills?';
      const longAnswer =
        'I think the most important aspect is understanding sentence structure and how different parts of speech work together to create clear, coherent, and natural-sounding English sentences.';
    ...
    it('should handle english-only scenario with line breaks', () => {
      const mockUser = createMockUser();
      const longTranslation =
        '今日は学校に行って英語の授業を受けました。とても興味深い内容で、特に文法の説明が分かりやすく、新しい表現もたくさん学ぶことができました。';
      const longVocabulary =
        'Key vocabulary includes: school (学校), classroom (教室), grammar (文法), expression (表現), interesting (興味深い), explanation (説明), and many other important terms for daily conversation.';
      const longGrammar =
        'This sentence uses past tense verbs and demonstrates the structure of compound sentences in English, showing how multiple ideas can be connected using conjunctions and appropriate punctuation.';
  ...
  describe('Content preservation', () => {
    it('should preserve all content when applying line breaks', () => {
      ...
      const originalTranslation =
        'This is a comprehensive translation that demonstrates various aspects of English grammar and vocabulary usage in natural conversational contexts.';
