...
export const bskyCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      console.log(
        `🐦 Processing Bluesky post request from ${interaction.user.tag}: "${message.substring(0, 50)}${message.length > 50 ? '...' : ''}"`
      );
      ...
      if (imageAttachment) {
        ...
        if (
          ...
          !validImageTypes.includes(imageAttachment.contentType)
        ) {
          await interaction.editReply({
            content:
      ...
      if (error instanceof Error) {
        if (error.message.includes('Rate limit')) {
          ...
          helpText =
            'Bluesky limits posts to prevent spam. Please wait a few minutes before posting again.';
        } else if (
          ...
          helpText =
            "There's an issue with the Bluesky account configuration. Please contact the bot administrator.";
