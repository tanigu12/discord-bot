...
export const formatCommand = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    try {
      ...
      console.log(
        `📋 Formatting thread content for ${interaction.user.tag} in thread: ${interaction.channel.name}`
      );
      console.log(
        `🔍 Thread ID: ${interaction.channel.id}, Parent: ${interaction.channel.parentId}`
      );
      console.log(`⚙️ Options: includeAll=${includeAll}, customTitle=${customTitle}`);

      // Read thread messages with options
      const threadData = await conversationReaderService.readThreadMessages(
        interaction.channel,
        includeAll
      );
      ...
      const channelInfo = interaction.channel.parent
        ? ` (from #${interaction.channel.parent.name})`
        : '';

      await interaction.editReply({
        content:
          `✨ **Thread formatted successfully!**\n\n` +
          `📊 **Statistics:**\n` +
