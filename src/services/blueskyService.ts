...
export class BlueskyService {
    ...
    try {
      ...
      if (error instanceof Error) {
        if (error.message.includes('Invalid identifier or password')) {
          throw new Error(
            'Invalid Bluesky credentials. Please check your username and app password.'
          );
        }
      ...
      throw new Error(
        `Bluesky authentication failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  async post(text: string, imageBuffer?: Buffer): Promise<BlueskyPostResult> {
    try {
      ...
      console.log(
        `📝 Preparing to post to Bluesky: "${text.substring(0, 50)}${text.length > 50 ? '...' : ''}"`
      );

      // Check text length (Bluesky limit is 300 characters)
      if (text.length > 300) {
        throw new Error(
          `Post too long (${text.length} characters). Bluesky limit is 300 characters.`
        );
      }
      ...
      if (error && typeof error === 'object' && 'status' in error) {
        ...
        if (status === 429) {
          throw new Error(
            'Rate limit exceeded. Please try again later (Bluesky allows 5,000 points per hour).'
          );
        }
        if (status === 400) {
      ...
      throw new Error(
        `Failed to post to Bluesky: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
